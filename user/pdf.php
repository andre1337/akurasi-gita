...
$pdf->Cell(5,1,'Nama User','LRTB',0,'C',1);
$pdf->Cell(3,1,'Telepon User','LRTB',0,'C',1);
$pdf->Cell(5,1,'Email','LRTB',0,'C',1);
$pdf->Cell(5,1,'Username','LRTB',0,'C',1);
$pdf->Ln();
$pdf->SetFont('Arial','','8');

for ($j=0;$j<$i;$j++){
  $pdf->Cell(1,1,$j+1,'LRTB',0,'L');         // no
  $pdf->Cell(3,1,$cell[$j][0],'LRTB',0,'L'); // idmhs
  $pdf->Cell(5,1,$cell[$j][1],'LRTB',0,'L'); // nama
  $pdf->Cell(3,1,$cell[$j][2],'LRTB',0,'L'); // telepon
  $pdf->Cell(5,1,$cell[$j][3],'LRTB',0,'L'); // email
  $pdf->Cell(5,1,$cell[$j][4],'LRTB',0,'L'); // username
  $pdf->Ln();
}
$pdf->Output();
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
